...
def grab_proxies():
    while True:
        all_proxies = requests.get(
            'https://api.proxyscrape.com/?request=getproxies&proxytype=http&timeout=5000&country=all&ssl=all&anonymity=all').text
...
def save(arg):
    ...
    with open('Images/' + arg.replace(half_url, '')[:6] + '.png', 'wb') as f:
        f.write(content)
